---
title: "🟢 Super, du hast deine eMail bestätigt."
layout: "mpf-block"
---
...
<form id="profileForm" class="space-y-3">
  <input
    ...
    name="email"
    readonly
    class="block w-full border border-gray-300 rounded-full bg-gray-100 text-center py-[10px] px-[14px] text-base text-gray-800"
  />
  <input
    type="text"
    id="firstName"
    ...
    required
    class="block w-full border border-gray-300 rounded-full py-[10px] px-[14px] text-base text-center placeholder-gray-400 placeholder:font-semibold"
  />
  <button
    type="submit"
    id="saveBtn"
    class="w-full bg-[#007aff] text-white px-4 py-2 rounded-full hover:bg-blue-700 transition duration-300"
  >
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (token) {
    try {
      ...
      if (confirmRes.ok && confirmData.success) {
        ...
        window.location.href = "/profil/link-abgelaufen/";
        return;
      }
    } catch (err) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    let res, payload;
    try {
      res = await fetch("https://api.puzh.com/update-profile.php", {
        method: "POST",
